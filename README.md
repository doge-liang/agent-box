...
ag-box 是给 AI coding agent(Claude Code / Codex / Grok)准备的 per-project 工作沙盒:每个项目一个盒,可在两个节点之间 park(归档)/ up(拉起)迁移,项目文件和会话记忆随迁移带走,**不做进程热迁移**。四层架构,各层各管一件事,ag-box CLI 本身只是薄编排层:
...
5. env 四行 S3 字段 + 密码就位后,**ag-box 本身不会自动 `restic init`**——新建的 R2 仓库要先手动 `restic init` 一次(用 env 里拼出的 `RESTIC_REPOSITORY=s3:$BOX_S3_ENDPOINT/$BOX_BUCKET/restic`),否则第一次 `ag-box track`/`ag-box snapshot` 会因仓库不存在而失败。
...
```bash
ag-box track myproj                          # /root/myproj 登记为沙盒,首次全量快照,租约在本机
ag-box attach myproj                         # 接入盒内 tmux,开始跑 agent
# ...干活、agent 写代码、git commit...
ag-box park myproj                           # 快照上传、释放租约、盒内进程停止
ssh my-second-node ag-box up myproj          # 另一节点拉起;不带 --node 时按各节点 MemAvailable 自动调度
ssh -t my-second-node ag-box attach myproj   # 接入,agent 里 --resume 续上原会话,记忆在位
```
...
- `restic snapshots --tag box:<名>` —— 查某个盒的快照历史(需要先把 `lib/env.js` 拼的 `RESTIC_REPOSITORY`/`RESTIC_PASSWORD`/`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` 四个变量 export 进当前 shell,或直接读 `/root/.config/box/env` 手动拼)。
- 盒起不来:先查 `/run/box/<名>/tmux.sock` 是否存在,再 `systemctl status boxrun-<名>` 看 unit 是不是直接 failed。
- 租约冲突(报错"沙盒活跃于 <节点>"):去持有租约的节点 `ag-box park <名>` 正常释放;确认对方确实已下线/失联,再用 `ag-box up <名> --force` 强制抢占(会警告分叉风险,对方未落盘的改动会在快照历史里分叉,以后到者为准)。

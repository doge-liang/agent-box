...
const { buildBwrapArgs, slugFor, bashProfileContent } = require('./mounts');
...
function startSandbox(cfg, meta) {
  ...
  // 盒内登录 shell 强制正确 PATH(堵宿主 attach 客户端 PATH 泄漏,见 mounts.bashProfileContent)
  fs.writeFileSync(`${boxHome(meta.name)}/.bash_profile`, bashProfileContent(!!meta.nix), { mode: 0o600 });

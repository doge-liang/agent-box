...
const { upgradeMeta, readLocalPaths, remoteMeta, writeLocalPaths } = require('./project');
...
function interpretMetaCat({ status, stdout, stderr }) {
  ...
  try {
    return upgradeMeta(JSON.parse(stdout));
  } catch {
...
function readMeta(cfg, name) {
  ...
  const meta = interpretMetaCat(r);
  if (!meta || !cfg.paths || !cfg.paths.configDir) return meta;
  return { ...meta, local_paths: readLocalPaths(cfg.paths.configDir, meta.project_id) };
}

function writeMeta(cfg, name, meta, runner = run) {
  const upgraded = upgradeMeta(meta);
  if (cfg.paths && cfg.paths.configDir) {
    writeLocalPaths(cfg.paths.configDir, upgraded.project_id, upgraded.local_paths);
  }
  runner('rclone', ['rcat', metaPath(cfg, name)],
    { env: cfg.rcloneEnv, input: JSON.stringify(remoteMeta(upgraded), null, 2) });
}
...
module.exports = {
  newMeta, readMeta, writeMeta, listBoxNames, iso, interpretMetaCat, interpretListResult, upgradeMeta,
};

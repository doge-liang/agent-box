...
// 远端清单不可信:拒绝越界路径段,防解密后的对象名逃逸落地目录
...
function init(pathArg, deps = {}) {
  ...
  if (map[id] && map[id] !== projectPath) {
    log(`提示: 项目路径由 ${map[id]} 更新为 ${projectPath}`);
    cfgMod.writeState(id, { version: 1, machines: {}, landed: {}, memory: {} });
    log('提示: 已重置该项目的同步状态(下次 pull 将全量重拉,memory 以保守冲突处理)');
  }
  map[id] = projectPath;
...
function pull(pathArg, deps = {}) {
  ...
  const mid = cfgMod.machineId();
  const state = cfgMod.readState(id);
  const origins = rcl.lsDirs(cfg, `${id}/claude`).filter((m) => m !== mid && isSafeRel(m) && !m.includes('/'));
  ...
  try {
    for (const origin of origins) {
      ...
      let remoteFiles = rcl.lsFiles(cfg, ns);
      const badRels = remoteFiles.filter((x) => !isSafeRel(x.rel));
      for (const x of badRels) log(`非法远端路径,已跳过: ${x.rel}`);
      if (badRels.length) remoteFiles = remoteFiles.filter((x) => isSafeRel(x.rel));

...
function createServer({ core, token, host = '127.0.0.1', tasks = new TaskManager() }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (!file.startsWith(WEB_ROOT) || !fs.existsSync(file)) return send(res, 404, { error: '不存在' });
    headers(res, MIME[path.extname(file)]); res.statusCode = 200; res.end(fs.readFileSync(file));
  });
  return { server, tasks };
}
